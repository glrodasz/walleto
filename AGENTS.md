...
- Borrado suave: `archived: true` en categorías, métodos de pago, cuentas y tags, `active: false` en transacciones recurrentes, `status: "SKIPPED"` en transacciones. Nunca `.delete()` sobre algo que otro doc referencia. Dos excepciones: `investmentValuations` (un punto de datos que nadie apunta, se borra de verdad) y **borrar la cuenta** (`DELETE /api/account` → `helpers/userData.ts`), que borra el grafo entero de un usuario porque el derecho de supresión no admite un soft delete. `USER_COLLECTIONS` en ese helper es el inventario de colecciones por usuario: una colección nueva se registra ahí o no se exporta ni se borra.
...
Qué regulación aplica, qué datos hay y qué falta: `docs/compliance.md`. La política pública vive en `/privacy` (`features/legal/components/PrivacyPage.tsx`) y tiene que decir lo mismo que ese documento.
